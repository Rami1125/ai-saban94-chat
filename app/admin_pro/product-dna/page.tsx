...
/**
 * Saban OS V74.1 - Product DNA Designer Studio (Refined v0 Edition)
 * ---------------------------------------------------------------
 * - Full Logic Integration: Connected to 'inventory' table.
 * - Live Simulator: High-fidelity Samsung Note 25 display.
 * - UX: Selection Sidebar + Executive DNA Editor.
 */
...
export default function ProductDNAStudio() {
  ...
  const handleSaveDNA = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("ה-DNA הזרק וסונכרן לשטח! 🦾", { id: toastId });
      fetchData();
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-10 font-sans text-right" dir="rtl">
      ...
      <div className="max-w-[1850px] mx-auto space-y-8">
          ...
          <div className="flex items-center gap-10 relative z-10">
             ...
             <div>
                ...
                <p className="text-[11px] font-bold text-slate-400 mt-3 uppercase tracking-[0.5em] flex items-center gap-2 justify-end">
                  Saban OS Elite V74.1 <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                </p>
             </div>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
          ...
          <div className="xl:col-span-9 grid grid-cols-1 lg:grid-cols-12 gap-10">
               ...
               <AnimatePresence mode="wait">
                 {designItem ? (
                      ...
                      <div className="space-y-12">
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            ...
                            <div className="space-y-3 text-right" dir="rtl">
                               ...
                               <select value={designItem.category} onChange={e => setDesignItem({...designItem, category: e.target.value})} className="w-full bg-slate-900 border-2 border-slate-800 p-5 rounded-[22px] font-black text-xl text-white outline-none focus:border-blue-500 transition-all appearance-none cursor-pointer">
                         ...
                         <div className="space-y-6 pt-10 border-t border-slate-800">
                            ...
                            {/* Slang Keywords (The Brain Engine) */}
                            <div className="space-y-3 text-right" dir="rtl">
                               <label className="text-[10px] font-black uppercase text-slate-500 mr-3 flex items-center gap-2 justify-end">
                                  מילון סלנג שטח (AI Retrieval) <Tag size={12}/>
                               </label>
                               <div className="flex gap-2">
                                  <input value={keywordInput} onChange={e => setKeywordInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addKeyword()} placeholder="הקלד מילה (למשל: בורג ורו) והקש Enter" className="flex-1 bg-slate-900 border-2 border-slate-800 p-5 rounded-[22px] font-black text-xl text-white outline-none focus:border-blue-500" />
                                  <button onClick={addKeyword} className="px-6 bg-blue-600 text-white rounded-2xl font-black transition-all active:scale-90">+</button>
                               </div>
                   ...
                   <div className="h-[900px] bg-white rounded-[70px] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center text-center p-20 shadow-inner group">
                      ...
                      <h3 className="text-4xl font-black text-slate-900 italic uppercase mb-6 tracking-tighter">Designer Ready</h3>
            ...
            {/* --- Live Simulator: Samsung Note 25 Frame --- */}
                  ...
                  <div className="flex-1 overflow-y-auto scrollbar-hide relative bg-white/5 backdrop-blur-3xl m-3 rounded-[55px] border border-white/5 p-6 space-y-6 text-white text-right" dir="rtl">
                     ...
                     <div className="space-y-4 pt-4 text-right">
                        <div className="flex items-center gap-3 justify-end">
                           <span className="bg-blue-600/20 text-blue-400 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border border-blue-500/20">Elite Series</span>
      ...
      <footer className="py-24 border-t border-slate-100 opacity-20 text-center uppercase text-[12px] font-black tracking-[3em] italic text-slate-900 leading-none">Saban OS Neural Design Engine V74.1</footer>
...
function DesignField({ label, value, type = "text", onChange, placeholder, disabled }: any) {
  return (
    <div className="space-y-3 text-right" dir="rtl">
       <label className="text-[11px] font-black text-slate-400 uppercase mr-3 tracking-[0.2em] italic block text-right">{label}</label>
       <input 
