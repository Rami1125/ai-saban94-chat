...
import { 
  ...
  Eye, Smartphone, ShieldCheck, Clock, Droplets, Hammer, Play
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { toast, Toaster } from "sonner";

/**
 * Saban Admin Pro - Catalog Studio Elite V34.0
 * -------------------------------------------
 * Fix: Removed unstable joins, fixed 'map' errors, added technical specs.
 * Permission Check: Ensure public.inventory and public.product_weights are writable.
 */
...
export default function CatalogStudio() {
  ...
  const enrichedItems = useMemo(() => {
    return items.map(item => {
      ...
      return { ...item, weight_kg: w?.weight_kg || 25, is_big_bag: w?.is_big_bag || false };
    });
  }, [items, weights]);
  ...
  const searchGoogle = async () => {
    ...
    try {
      ...
      setGoogleResults(Array.isArray(data) ? data : []); // הגנה מפני קריסה
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("DNA הזרק בהצלחה! 🦾", { id: toastId });
      setEditingItem(null);
      fetchData();
    } catch (err) {
      toast.error("וודא הרשאות כתיבה בטבלאות", { id: toastId });
    }
  };
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10">
           <div className="flex items-center gap-8">
              ...
              <div className="text-right">
                ...
                <p className="text-blue-300/80 text-xs font-bold uppercase tracking-[0.3em]">Elite DNA & UI Simulator</p>
              </div>
           </div>
        ...
        <div className="lg:col-span-4 h-[850px]">
          <div className="bg-white rounded-[45px] border border-slate-200 shadow-xl overflow-hidden flex flex-col h-full">
             <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <h3 className="font-black text-slate-800 uppercase text-xs flex items-center gap-3 tracking-widest"><Layers size={18} className="text-blue-600"/> מאגר ({filteredItems.length})</h3>
                ...
                {filteredItems.map(item => (
                  ...
                  >
                    <div className="flex items-center gap-5">
                       <div className="w-14 h-14 rounded-2xl overflow-hidden bg-slate-50 flex items-center justify-center border border-slate-200">
        ...
        <div className="lg:col-span-8">
           <AnimatePresence mode="wait">
             {editingItem ? (
               <motion.div key="studio" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                  ...
                  <div className="bg-white rounded-[50px] border border-slate-200 shadow-2xl p-8 md:p-10 space-y-8 overflow-y-auto max-h-[850px] scrollbar-hide">
                     <div className="flex justify-between items-center border-b pb-6">
                        <div className="flex items-center gap-4"><Palette className="text-blue-600"/><h3 className="text-xl font-black uppercase italic">Elite Editor</h3></div>
                     ...
                     <div className="grid grid-cols-3 gap-4 bg-blue-50/50 p-5 rounded-[30px]">
                        <div className="space-y-1"><label className="text-[9px] font-black text-blue-600 uppercase italic flex items-center gap-1"><Clock size={10}/> ייבוש</label><input value={editingItem.drying_time || ""} onChange={e => setEditingItem({...editingItem, drying_time: e.target.value})} className="w-full bg-white p-3 rounded-xl font-bold text-xs text-right" /></div>
                        <div className="space-y-1"><label className="text-[9px] font-black text-blue-600 uppercase italic flex items-center gap-1"><Calculator size={10}/> כיסוי</label><input value={editingItem.coverage_info || ""} onChange={e => setEditingItem({...editingItem, coverage_info: e.target.value})} className="w-full bg-white p-3 rounded-xl font-bold text-xs text-right" /></div>
                        <div className="space-y-1"><label className="text-[9px] font-black text-blue-600 uppercase italic flex items-center gap-1"><Hammer size={10}/> יישום</label><input value={editingItem.application_method || ""} onChange={e => setEditingItem({...editingItem, application_method: e.target.value})} className="w-full bg-white p-3 rounded-xl font-bold text-xs text-right" /></div>
                     </div>

                     <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 mr-2 uppercase italic flex items-center gap-2"><Video size={14}/> לינק יוטיוב</label><input value={editingItem.video_url || ""} onChange={e => setEditingItem({...editingItem, video_url: e.target.value})} className="w-full bg-slate-50 p-4 rounded-2xl font-bold text-sm text-left" /></div>

                     {/* Google Asset Picker */}
                     <div className="bg-[#0F172A] rounded-[35px] p-6 space-y-4">
                        <div className="flex justify-between items-center"><h4 className="text-blue-400 font-black text-xs uppercase italic tracking-widest flex items-center gap-2"><Globe size={14}/> Google Search</h4></div>
                        ...
                        <div className="grid grid-cols-3 gap-3 max-h-[180px] overflow-y-auto scrollbar-hide">
                           {googleResults.map((res, i) => (
                             ...
                             }} className="aspect-square rounded-xl overflow-hidden border-2 border-transparent hover:border-blue-500 transition-all shadow-lg"><img src={res.link} className="w-full h-full object-cover" /></button>
                           ))}
                        </div>
                        <button onClick={() => setEditingItem({...editingItem, image_url:'', image_url_2:'', image_url_3:''})} className="text-[9px] text-slate-500 uppercase font-black hover:text-white">נקה מדיה</button>
                     </div>
                     ...
                     <div className="bg-[#0F172A] rounded-[60px] overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.5)] border border-white/5 sticky top-10 w-full max-w-[420px]">
                        <div className="p-8">
                           <div className="flex gap-3 h-56">
                              <div className="flex-[2] bg-slate-800 rounded-[30px] overflow-hidden relative border border-white/10">{editingItem.image_url ? <img src={editingItem.image_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-700"><ImageIcon size={40}/></div>}</div>
                              <div className="flex-1 flex flex-col gap-3">
                                 <div className="flex-1 bg-slate-800 rounded-[20px] overflow-hidden border border-white/10">{editingItem.image_url_2 ? <img src={editingItem.image_url_2} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-700"><ImageIcon size={16}/></div>}</div>
                                 <div className="flex-1 bg-slate-800 rounded-[20px] overflow-hidden border border-white/10">{editingItem.image_url_3 ? <img src={editingItem.image_url_3} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-700"><ImageIcon size={16}/></div>}</div>
                              </div>
                           </div>
                        </div>
                        
                        {getYoutubeId(editingItem.video_url) && (
                          <div className="px-8 pb-6"><div className="aspect-video bg-black rounded-[25px] overflow-hidden border border-white/5 shadow-2xl"><iframe className="w-full h-full" src={`https://www.youtube.com/embed/${getYoutubeId(editingItem.video_url)}?modestbranding=1`} frameBorder="0" allowFullScreen /></div></div>
                        )}

                        <div className="px-10 pb-10 space-y-6">
                           <div className="text-right">
                              <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase leading-none mb-2">{editingItem.product_name || "Product Name"}</h2>
                              <span className="bg-blue-600 text-white px-3 py-1 rounded-lg text-[9px] font-black uppercase italic tracking-widest shadow-lg">SKU {editingItem.sku || "0000"}</span>
                           </div>
                           ...
                           <div className="bg-blue-600/10 border border-blue-500/20 p-6 rounded-[30px] relative overflow-hidden">
                              ...
                              <p className="text-white text-xs font-bold leading-relaxed text-right opacity-80 italic">"יישום: {editingItem.application_method || 'נא להגדיר שיטת יישום בדף המנהל'}"</p>
                           </div>
               ...
               <div className="h-[850px] bg-white rounded-[60px] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center text-center p-20 shadow-inner group"><div className="w-32 h-32 bg-slate-50 rounded-[45px] flex items-center justify-center text-slate-200 mb-8 border-4 border-white shadow-xl group-hover:scale-110 transition-transform"><Palette size={64} /></div><h3 className="text-3xl font-black text-slate-900 italic uppercase mb-4 tracking-tighter">Saban Catalog Studio</h3><p className="text-slate-400 max-w-sm font-bold text-sm leading-relaxed uppercase tracking-widest italic opacity-60">בחר מוצר כדי להתחיל לערוך את ה-DNA הדיגיטלי שלו. המערכת תסנכרן את הכרטיס המלא לפורטל ה-VIP.</p></div>
             )}
           </AnimatePresence>
        </div>
      </div>
      <footer className="py-12 border-t border-slate-200 opacity-20 text-center uppercase text-[10px] font-black tracking-[0.8em] text-slate-900">Saban Catalog Studio V34.0</footer>
    </div>
  );
}
