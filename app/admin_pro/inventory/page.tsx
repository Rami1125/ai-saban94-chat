...
import { 
  ...
  Clock, Hammer, Play, Eye, Smartphone, AlertTriangle
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { toast, Toaster } from "sonner";

/**
 * Saban Admin Pro - Catalog Studio Elite V41.0
 * -------------------------------------------
 * - Fix: Removed all external via.placeholder URLs to prevent DNS errors.
 * - Fix: Null-safe filtering to prevent 'includes' crashes.
 * - Features: Professional Specs, YouTube Player, Triple Image Layout.
 */
...
export default function CatalogStudio() {
  ...
  const handleSave = async () => {
    ...
    // ניקוי לינקים שבורים לפני שמירה
    const cleanImageUrl = (url: string) => (url?.includes('via.placeholder') || !url) ? "" : url;

    try {
      const { error: invErr } = await supabase.from('inventory').upsert({
        sku: editingItem.sku,
        product_name: editingItem.product_name || "",
        image_url: cleanImageUrl(editingItem.image_url),
        image_url_2: cleanImageUrl(editingItem.image_url_2),
        image_url_3: cleanImageUrl(editingItem.image_url_3),
  ...
  // מנגנון סינון חסין קריסה (Null-Safe)
  ...
  // פונקציית עזר להצגת תמונה או פלייסולדר מקומי
  const ImageFallback = ({ url, className }: { url: string, className: string }) => {
    const isInvalid = !url || url.includes('via.placeholder');
    if (isInvalid) return (
      <div className={`${className} bg-slate-100 flex flex-col items-center justify-center text-slate-300 gap-2 border border-slate-200`}>
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10">
           <div className="flex items-center gap-8">
              ...
              <div className="text-right">
                ...
                <p className="text-blue-300/80 text-xs font-bold uppercase tracking-[0.3em]">Visual Identity & Technical DNA</p>
              </div>
           </div>
           ...
           <AnimatePresence mode="wait">
             {editingItem ? (
               <motion.div key="studio" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                  ...
                  {/* Editor Card */}
                  <div className="bg-white rounded-[50px] border border-slate-200 shadow-2xl p-8 md:p-10 space-y-8 overflow-y-auto max-h-[850px] scrollbar-hide text-right">
                     <div className="flex justify-between items-center border-b pb-8">
                        <div className="flex items-center gap-4">
                           <Palette className="text-blue-600" size={32}/>
                           <h3 className="text-2xl font-black uppercase italic leading-none text-slate-900">Elite Designer</h3>
                        </div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-blue-50/50 p-6 rounded-[35px] border border-blue-100 shadow-inner">
                        <div className="space-y-1"><label className="text-[9px] font-black text-blue-600 uppercase italic flex items-center gap-1 justify-end"><Clock size={10}/> ייבוש</label><input value={editingItem.drying_time || ""} onChange={e => setEditingItem({...editingItem, drying_time: e.target.value})} className="w-full bg-white p-3 rounded-xl font-bold text-xs text-right outline-none" /></div>
                        <div className="space-y-1"><label className="text-[9px] font-black text-blue-600 uppercase italic flex items-center gap-1 justify-end"><Calculator size={10}/> כיסוי</label><input value={editingItem.coverage_info || ""} onChange={e => setEditingItem({...editingItem, coverage_info: e.target.value})} className="w-full bg-white p-3 rounded-xl font-bold text-xs text-right outline-none" /></div>
                        <div className="space-y-1"><label className="text-[9px] font-black text-blue-600 uppercase italic flex items-center gap-1 justify-end"><Hammer size={10}/> יישום</label><input value={editingItem.application_method || ""} onChange={e => setEditingItem({...editingItem, application_method: e.target.value})} className="w-full bg-white p-3 rounded-xl font-bold text-xs text-right outline-none" /></div>
                     </div>

                     <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 mr-2 uppercase italic flex items-center gap-2 justify-end"><Video size={14} className="text-rose-500"/> לינק יוטיוב למדריך</label><input value={editingItem.video_url || ""} onChange={e => setEditingItem({...editingItem, video_url: e.target.value})} placeholder="https://youtube.com/..." className="w-full bg-slate-50 p-4 rounded-2xl font-bold text-sm text-left" /></div>
                     ...
                     <div className="bg-[#0F172A] rounded-[35px] p-8 space-y-6 relative overflow-hidden shadow-2xl">
                        ...
                        <div className="flex justify-between items-center relative z-10"><h4 className="text-blue-400 font-black text-xs uppercase italic tracking-widest flex items-center gap-2"><Globe size={16}/> Media Engine</h4></div>
                        <button onClick={searchGoogle} disabled={isSearchingGoogle} className="w-full bg-white py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-xl hover:bg-blue-50 transition-all flex items-center justify-center gap-4 relative z-10 active:scale-95">
                        ...
                        <div className="grid grid-cols-3 gap-4 max-h-[220px] overflow-y-auto scrollbar-hide p-1 relative z-10">
                           {googleResults.map((res, i) => (
                             ...
                             }} className="aspect-square rounded-[22px] overflow-hidden border-4 border-transparent hover:border-blue-500 transition-all relative group shadow-lg bg-slate-800"><img src={res.link} className="w-full h-full object-cover" /><div className="absolute inset-0 bg-blue-600/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white"><Plus size={32}/></div></button>
                           ))}
                        </div>
                     ...
                     <motion.div layout className="bg-slate-900 rounded-[70px] overflow-hidden shadow-[0_60px_120px_rgba(0,0,0,0.5)] border border-white/5 sticky top-10 w-full max-w-[440px]">
                        ...
                        <div className="p-10">
                           <div className="flex gap-4 h-64">
                              <div className="flex-[2] bg-slate-800 rounded-[35px] overflow-hidden relative border border-white/10 group">
                                 ...
                                 <div className="absolute top-5 left-5 bg-blue-600 text-white px-4 py-1.5 rounded-xl text-[10px] font-black uppercase shadow-lg animate-pulse tracking-widest">Main</div>
                              </div>
                        ...
                        <div className="px-12 pb-12 space-y-10 text-right" dir="rtl">
                           <div className="space-y-3">
                              <h2 className="text-3xl md:text-4xl font-black text-white italic tracking-tighter uppercase leading-none">{editingItem.product_name || "שם מוצר Elite"}</h2>
                              <div className="flex items-center gap-4">
                                 ...
                                 <span className="bg-emerald-500/20 text-emerald-400 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border border-emerald-500/20 flex items-center gap-2"><ShieldCheck size={12}/> Saban Approved</span>
                              </div>
                           </div>
                           ...
                           <div className="bg-blue-600/10 border border-blue-500/20 p-8 rounded-[45px] relative overflow-hidden shadow-inner group">
                              ...
                              <div className="relative z-10 flex items-center justify-between mb-5">
                                 <span className="text-[11px] font-black text-blue-400 uppercase italic tracking-widest flex items-center gap-3"><Sparkles size={16} className="animate-pulse"/> Technical Advisor OS</span>
                              ...
                              <p className="text-white text-[16px] font-bold leading-relaxed text-right opacity-80 italic">
                                 "{editingItem.application_method || 'נא להגדיר שיטת יישום מומלצת'}"
                              </p>
                           </div>
                           ...
                           <button className="w-full bg-white text-slate-900 py-8 rounded-[40px] font-black text-xs uppercase tracking-[0.5em] shadow-2xl flex items-center justify-center gap-6 border-b-8 border-slate-200 active:scale-95 transition-all italic ring-[15px] ring-white/5 hover:bg-blue-50">
                              הוסף להזמנה <ArrowUpRight size={26} className="text-blue-600" />
                           </button>
                        </div>
                     </motion.div>
                  </div>
               ...
               <div className="h-[850px] bg-white rounded-[60px] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center text-center p-20 shadow-inner group">
                  ...
                  <h3 className="text-4xl font-black text-slate-900 italic uppercase mb-6 tracking-tighter">Saban Catalog Studio</h3>
                  <p className="text-slate-400 max-w-md font-bold text-base leading-relaxed uppercase tracking-widest italic opacity-60">בחר מוצר מרשימת המלאי כדי להתחיל לערוך את ה-DNA שלו. המערכת תסנכרן את הכרטיס המלא לפורטל ה-VIP.</p>
               </div>
             )}
           </AnimatePresence>
        </div>
      </div>
      <footer className="py-20 border-t border-slate-200 opacity-20 text-center uppercase text-[11px] font-black tracking-[1em] text-slate-900 text-right pr-10">Saban Visual Intelligence V41.0</footer>
