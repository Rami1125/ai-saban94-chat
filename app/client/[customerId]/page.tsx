...
import { 
  ...
  ArrowLeftRight, PackagePlus, Loader2, PlusSquare // <--- הייבוא החסר
} from "lucide-react";
...
export default function SabanClientAirApp() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
        // 1. משיכת פרטי לקוח
        const { data: client } = await supabase.from('saban_customers').select('*').eq('customer_id', cleanId).maybeSingle();
        setCustomer(client);

        // 2. משיכת הזמנות (עם טיפול בשגיאת 400 אם הטבלה לא מעודכנת)
        const { data: ordersData, error } = await supabase.from('saban_master_dispatch')
          .select('*')
  ...
  const sendQuickRequest = async (action: string, orderDetails?: any) => {
    ...
    if (!error) {
        toast.dismiss();
        toast.success("נשלח לראמי בסידור! תקבל התראה כשיאושר.");
    } else {
        toast.dismiss();
    ...
    <div className="min-h-screen bg-[#FDFDFD] text-slate-900 font-sans pb-32" dir="rtl">
      ...
      <header className="sticky top-0 z-[100] bg-white/70 backdrop-blur-xl border-b border-slate-100 p-5 flex items-center justify-between">
        ...
        <div className="text-center">
            <h1 className="text-2xl font-black italic text-blue-700 tracking-tighter leading-none uppercase">ח. סבן</h1>
            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-1 italic leading-none">Client Portal</p>
        </div>
          ...
          <div className="fixed inset-0 z-[200] bg-black/30 backdrop-blur-sm transition-all flex justify-end">
              <aside className="w-80 bg-white h-full shadow-2xl p-8 animate-in slide-in-from-right duration-300 flex flex-col">
                  ...
                  <nav className="space-y-4 flex-1">
                      <button onClick={() => {setActiveTab('home'); setIsMenuOpen(false);}} className={`w-full flex items-center gap-4 p-5 rounded-[2rem] font-black transition-all ${activeTab === 'home' ? 'bg-blue-600 text-white shadow-xl shadow-blue-100' : 'text-slate-400 hover:bg-slate-50'}`}>
                          <LayoutGrid size={22}/> מרכז בקרה
                      </button>
                      <button onClick={() => {setActiveTab('history'); setIsMenuOpen(false);}} className={`w-full flex items-center gap-4 p-5 rounded-[2rem] font-black transition-all ${activeTab === 'history' ? 'bg-blue-600 text-white shadow-xl shadow-blue-100' : 'text-slate-400 hover:bg-slate-50'}`}>
                          <History size={22}/> היסטוריה
                      </button>
                      <button onClick={() => {setActiveTab('chat'); setIsMenuOpen(false);}} className={`w-full flex items-center gap-4 p-5 rounded-[2rem] font-black transition-all ${activeTab === 'chat' ? 'bg-blue-600 text-white shadow-xl shadow-blue-100' : 'text-slate-400 hover:bg-slate-50'}`}>
                          <MessageSquare size={22}/> צ'אט פקודות AI
                      </button>
                  </nav>
              </aside>
          </div>
      ...
      <main className="p-6 max-w-xl mx-auto space-y-8">
        {activeTab === 'home' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                <section>
                    <h2 className="text-3xl font-black text-slate-800 italic tracking-tight uppercase leading-none">שלום, {customer?.full_name?.split(' ')[0] || 'לקוח'} 👋</h2>
                    <p className="text-xs font-bold text-slate-400 mt-1 uppercase italic tracking-widest">{customer?.address || 'חברת ח. סבן'}</p>
                </section>

                <section className="space-y-4">
                    <div className="flex items-center justify-between px-2">
                        <h3 className="font-black italic text-slate-800">פעילות חיה בשטח</h3>
                        <div className="flex items-center gap-2"><div className="w-2 h-2 bg-green-500 rounded-full animate-ping"></div><span className="text-[10px] font-black text-green-600 uppercase">Live</span></div>
                    </div>
                    {orders.filter(o => o.status !== 'הושלם').map(order => (
                        <Card key={order.id} className="p-6 rounded-[2.5rem] bg-white border-none shadow-xl shadow-slate-200/50 relative overflow-hidden ring-1 ring-slate-100">
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-4 border-t border-slate-50 pt-6 font-black italic">
                                <button onClick={() => sendQuickRequest('החלפה', order)} className="bg-slate-50 text-slate-600 p-4 rounded-2xl flex flex-col items-center gap-1 hover:bg-blue-50 hover:text-blue-700 transition-all text-[10px] border-none font-black">
                                    <RefreshCcw size={18}/> בקש החלפה
                                </button>
                                <button onClick={() => sendQuickRequest('פינוי', order)} className="bg-slate-50 text-slate-600 p-4 rounded-2xl flex flex-col items-center gap-1 hover:bg-red-50 hover:text-red-600 transition-all text-[10px] border-none font-black">
                    ...
                    {orders.filter(o => o.status !== 'הושלם').length === 0 && (
                        <Card className="p-10 rounded-[2.5rem] bg-slate-50/50 border-dashed border-2 border-slate-200 flex flex-col items-center justify-center text-slate-300">
                            ...
                            <p className="font-black italic text-sm italic">אין עבודות פעילות כרגע</p>
                        </Card>
                    )}
                </section>
                ...
                <section className="grid grid-cols-2 gap-4">
                    <button onClick={() => sendQuickRequest('הצבה')} className="bg-white p-6 rounded-[2.5rem] shadow-sm flex flex-col items-center gap-3 border border-slate-100 hover:shadow-xl transition-all group">
                        ...
                        <span className="font-black italic text-xs italic uppercase">הזמנה חדשה</span>
                    </button>
                    <button onClick={() => setActiveTab('history')} className="bg-white p-6 rounded-[2.5rem] shadow-sm flex flex-col items-center gap-3 border border-slate-100 hover:shadow-xl transition-all group font-black">
                        ...
                        <span className="font-black italic text-xs uppercase">היסטוריה</span>
                    </button>
                </section>
            </div>
        )}
      ...
      <nav className="fixed bottom-8 left-8 right-8 z-[150]">
        <div className="bg-white/80 backdrop-blur-2xl border border-white/50 rounded-[3rem] p-4 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] flex justify-around items-center px-12 ring-8 ring-black/5">
            <button onClick={() => setActiveTab('home')} className={`flex flex-col items-center gap-1 transition-all border-none bg-transparent ${activeTab === 'home' ? 'text-blue-700 scale-110' : 'text-slate-300'}`}>
                <LayoutGrid size={24} />
            </button>
            <button onClick={() => setActiveTab('chat')} className="relative -mt-16 bg-transparent border-none outline-none cursor-pointer">
            ...
            <button onClick={() => setActiveTab('history')} className={`flex flex-col items-center gap-1 transition-all border-none bg-transparent ${activeTab === 'history' ? 'text-blue-700 scale-110' : 'text-slate-300'}`}>
                <History size={24} />
            </button>
        </div>
      </nav>
