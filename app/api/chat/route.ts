...
export async function POST(req: Request) {
  try {
    ...
    // 3. חיפוש מלאי משופר (כאן שונה מ-textSearch ל-ilike לשליפה מדויקת)
    ...
    if (foundProduct) {
      ...
      productContext = `מוצר נמצא: ${foundProduct.product_name} | מק"ט (SKU): ${foundProduct.sku}`;
    }
        ...
        try {
          const model = genAI.getGenerativeModel({
            ...
            systemInstruction: `
              ...
              נתוני מוצר: ${productContext}
              מלאי: ${stockAlert}
              חוק: אם נמצא מוצר, סיים חובה במילה: MAGIC_URL
              חתימה: H.SABAN 1994
            `
          });
    ...
    // 6. הזרקת לינקים (שליפת המק"ט לתוך הלינק)
