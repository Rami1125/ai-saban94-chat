...
export async function POST(req: Request) {
  try {
    ...
    const { question, messages, userName, travelInfo: travelInfoFromMsg } = body;
    ...
    const travelData = travelInfoFromMsg || { distance: "טרם נקבע", duration: "בחישוב..." };
    ...
    // הכנת נתוני הניווט עבור ה-DNA
    const travelInfo = typeof travelData !== 'undefined' ? travelData : { distance: "טרם נקבע", duration: "בחישוב..." };

    // --- 2. בניית ה-DNA המאוחד (Saban OS V8.2) ---
    // שים לב: המחרוזת נפתחת כאן ונסגרת רק בסוף כל ההנחיות
