...
// הגדרות OneSignal - וודא שה-App ID שלך מעודכן כאן
const ONESIGNAL_APP_ID = "YOUR_ONESIGNAL_APP_ID"; // תחליף ב-ID האמיתי שלך
const ONESIGNAL_REST_KEY = "YOUR_REST_API_KEY"; // מומלץ להעביר ל-Environment Variable
...
export default function SabanOSVipDispatch() {
  ...
  // פונקציה לשליחת התראה דרך OneSignal
  const sendPushNotification = async (orderData: any) => {
    try {
      const response = await fetch("https://onesignal.com/api/v1/notifications", {
        ...
        body: JSON.stringify({
          ...
          included_segments: ["All"], // שולח לכולם
          headings: { "en": "📦 הזמנה חדשה בסידור", "he": "📦 הזמנה חדשה בסידור" },
          contents: { 
            "en": `Customer: ${orderData.customer_name} | Driver: ${orderData.driver_name}`,
            "he": `לקוח: ${orderData.customer_name} | נהג: ${orderData.driver_name} | שעה: ${orderData.scheduled_time}` 
          },
          data: { orderId: orderData.order_id_comax },
          android_accent_color: "0B2C63",
          priority: 10
        })
      });
                ...
                <Card key={order.id} className="border-none shadow-md rounded-2xl bg-white overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  {expandedId === order.id && (
                    <div className="p-4 bg-slate-50 border-t border-slate-100 space-y-3">
                      <div className="grid grid-cols-2 gap-2 text-[11px] font-bold text-slate-500 text-right" dir="rtl">
                      ...
                      <div className="flex justify-end gap-2 pt-2 border-t border-slate-200">
                         <Button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent('עדכון על הזמנה ' + order.order_id_comax)}`, '_blank')} variant="ghost" className="text-blue-600 h-8 text-[10px] font-black hover:bg-blue-50 border-none">
                           <Share2 size={14} className="ml-1"/> שתף שוב
                         </Button>
                         <Button onClick={() => {if(confirm('למחוק הזמנה?')) supabase.from('saban_dispatch').delete().eq('id', order.id).then(fetchOrders)}} variant="ghost" className="text-red-500 h-8 text-[10px] font-black hover:bg-red-50 border-none">
        ...
        <div className="fixed inset-0 bg-[#0B2C63]/90 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <Card className="bg-white w-full max-w-lg rounded-[2rem] p-6 space-y-4 shadow-2xl border-t-8 border-blue-600">
            ...
            <div className="flex gap-2 mb-2">
                {teamMembers.map(m => (
                    <button key={m} onClick={() => setNewOrder({...newOrder, created_by: m})}
                            className={`flex-1 py-2 rounded-xl text-[10px] font-black transition-all ${newOrder.created_by === m ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500 border-none outline-none'}`}>
                        {m}
                    </button>
                ))}
            </div>
            ...
            <input placeholder="שם הלקוח" value={newOrder.customer_name} onChange={e => setNewOrder({...newOrder, customer_name: e.target.value})} 
                   className="w-full h-12 px-4 rounded-xl border-2 border-slate-100 font-bold focus:border-blue-500 outline-none text-right" dir="rtl" />
            ...
            <div className="grid grid-cols-2 gap-3">
                <input placeholder="מספר קומקס" value={newOrder.order_id_comax} onChange={e => setNewOrder({...newOrder, order_id_comax: e.target.value})} 
                       className="h-12 px-4 rounded-xl border-2 border-slate-100 font-bold focus:border-blue-500 outline-none text-right" dir="rtl" />
                <select value={newOrder.warehouse_source} onChange={e => setNewOrder({...newOrder, warehouse_source: e.target.value})} 
                        className="h-12 px-4 rounded-xl border-2 border-slate-100 font-bold bg-white focus:border-blue-500 outline-none text-right" dir="rtl">
