...
import { 
  ...
  Calendar, Clock, Warehouse, MapPin, Share2, Bot, UserCheck, HardHat, Recycle, ClipboardList
} from "lucide-react";
import { toast, Toaster } from "sonner";

// הוספת נהג וירטואלי "פינוי פסולת"
const drivers = [
...
const actionTypes = ['הצבה', 'החלפה', 'הוצאה'];
...
export default function SabanOSVipDispatch() {
  ...
  const [newOrder, setNewOrder] = useState({
    ...
    container_action: 'הצבה', // שדה חדש למכולות
  });
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-24 font-sans text-right" dir="rtl">
      ...
      <div className="bg-[#0B2C63] text-white p-6 rounded-b-[2.5rem] shadow-2xl mb-6 border-b-4 border-blue-500/50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-black italic tracking-tighter text-white uppercase">SABAN<span className="text-blue-400">OS</span></h1>
            <p className="text-[10px] text-blue-200 font-bold uppercase tracking-widest">Logistic Control Center</p>
          </div>
          <Button onClick={() => setShowForm(true)} className="bg-blue-600 hover:bg-blue-500 rounded-xl gap-2 font-black shadow-lg h-12 px-6 text-white border-none">
            ...
            <Card className="bg-white p-5 rounded-[2rem] shadow-xl border border-slate-100 relative overflow-hidden">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <Badge className={`${driver.name === 'פינוי פסולת' ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700'} border-none font-bold text-[10px]`}>
                      {driver.defaultType}
                  </Badge>
                </div>
              </div>
                ...
                <Card key={order.id} className="border-none shadow-md rounded-2xl bg-white overflow-hidden hover:shadow-lg transition-shadow">
                  <div onClick={() => setExpandedId(expandedId === order.id ? null : order.id)} className="p-4 flex justify-between items-center cursor-pointer">
                    <div className="flex items-center gap-3">
                      <div className={`${driver.name === 'פינוי פסולת' ? 'bg-green-600' : 'bg-[#0B2C63]'} text-white font-black px-2 py-1 rounded-lg text-[10px] italic`}>
                  ...
                  {expandedId === order.id && (
                    <div className="p-4 bg-slate-50 border-t border-slate-100 space-y-3">
                      <div className="grid grid-cols-2 gap-2 text-[11px] font-bold text-slate-500">
                        <div className="flex items-center gap-1 justify-end">{order.address || 'ללא כתובת'} <MapPin size={12}/></div>
                      ...
                      <div className="flex justify-end gap-2 pt-2 border-t border-slate-200">
                         <Button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent('עדכון על ' + order.order_id_comax)}`, '_blank')} variant="ghost" className="text-blue-600 h-8 text-[10px] font-black border-none">
                           שתף <Share2 size={14} className="mr-1"/>
                         </Button>
                         <Button onClick={() => {if(confirm('למחוק?')) supabase.from('saban_dispatch').delete().eq('id', order.id).then(fetchOrders)}} variant="ghost" className="text-red-500 h-8 text-[10px] font-black border-none">
                           מחק <Trash2 size={14} className="mr-1"/>
                         </Button>
                      </div>
                    </div>
                  )}
      ...
      {/* טופס יצירה - תומך מכולות */}
      {showForm && (
        <div className="fixed inset-0 bg-[#0B2C63]/90 backdrop-blur-sm z-[100] flex items-center justify-center p-4 text-right" dir="rtl">
          <Card className="bg-white w-full max-w-lg rounded-[2rem] p-6 space-y-4 shadow-2xl border-t-8 border-blue-600 animate-in zoom-in-95">
            ...
            <div className="flex gap-2 mb-2">
                {teamMembers.map(m => (
                    <button key={m} onClick={() => setNewOrder({...newOrder, created_by: m})}
                            className={`flex-1 py-2 rounded-xl text-[10px] font-black transition-all ${newOrder.created_by === m ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500 border-none'}`}>
                        {m}
                    </button>
                ))}
            </div>

            {/* שדות ייחודיים למכולות */}
            {newOrder.driver_name === 'פינוי פסולת' && (
                <div className="space-y-4 bg-green-50 p-4 rounded-2xl border border-green-100">
                    <div>
                        <label className="text-[10px] font-black text-green-700 mr-1">סוג פעולה מכולה</label>
                        <div className="grid grid-cols-3 gap-2 mt-1">
                            {actionTypes.map(type => (
                                <button key={type} onClick={() => setNewOrder({...newOrder, container_action: type})}
                                        className={`py-2 rounded-xl text-[11px] font-black border-2 transition-all ${newOrder.container_action === type ? 'bg-green-600 border-green-600 text-white' : 'bg-white border-green-200 text-green-600'}`}>
                                    {type}
                                </button>
                            ))}
                        </div>
            ...
            <input placeholder="שם הלקוח" value={newOrder.customer_name} onChange={e => setNewOrder({...newOrder, customer_name: e.target.value})} 
                   className="w-full h-12 px-4 rounded-xl border-2 border-slate-100 font-bold focus:border-blue-500 outline-none text-right" />
            ...
            <div className="grid grid-cols-2 gap-3">
                <input placeholder="מספר קומקס" value={newOrder.order_id_comax} onChange={e => setNewOrder({...newOrder, order_id_comax: e.target.value})} 
                       className="h-12 px-4 rounded-xl border-2 border-slate-100 font-bold focus:border-blue-500 outline-none text-right" />
                
                {newOrder.driver_name === 'פינוי פסולת' ? (
                    <select value={newOrder.warehouse_source} onChange={e => setNewOrder({...newOrder, warehouse_source: e.target.value})} 
                            className="h-12 px-4 rounded-xl border-2 border-slate-100 font-bold bg-white focus:border-blue-500 outline-none text-right">
            ...
            {/* בחירת סוג הובלה (רק אם לא מכולה) */}
            {newOrder.driver_name !== 'פינוי פסולת' && (
                <div className="grid grid-cols-2 gap-3">
            ...
            <Button onClick={saveOrderAndShare} className="w-full h-14 bg-green-600 hover:bg-green-500 rounded-xl font-black text-lg text-white shadow-xl border-none">
              שמור ושתף לערוץ ✨
            </Button>
          </Card>
        </div>
