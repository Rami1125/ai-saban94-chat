...
// שים לב לייבוא המתוקן כאן - הוספתי את Scan ו-MessageSquare
import { 
  Sparkles, 
  ShoppingCart, 
  Palette, 
          ...
          <Dialog>
            ...
            <DialogContent className="bg-white rounded-[2rem] max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 gap-3 p-2">
                {products.map(p => (
                  <button key={p.id} onClick={() => addToCart(p)} className="flex justify-between p-4 bg-slate-50 rounded-2xl font-bold hover:bg-blue-50 transition-colors text-right">
                    <span>{p.name}</span>
