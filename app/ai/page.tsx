...
import { 
  ...
  Clock, Warehouse, MapPin, Share2, UserCheck, HardHat, Recycle, Menu, Edit2, Calendar, RefreshCw, Brain, Loader2
} from "lucide-react";
...
export default function SabanMasterDispatch() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-24 font-sans text-right" dir="rtl">
      ...
      {/* 🔥 Desktop Permanent AI Sidebar (מתחת להזמנות) */}
      <div className="hidden lg:block max-w-[1800px] mx-auto px-4 mt-12">
        <Card className="bg-[#0B2C63] rounded-[3rem] p-8 shadow-2xl border-none relative overflow-hidden">
          ...
          <div className="flex items-center gap-6 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-black text-white italic uppercase tracking-tighter">Saban AI Dispatch</h2>
              <p className="text-blue-300 text-sm font-bold uppercase tracking-widest">מערכת פקודות חכמה - שליטה ב-SQL</p>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <input 
              ...
              placeholder="ראמי, מה הפקודה? (למשל: 'תעביר את אדר בניה לשעה 11:00')"
              className="flex-1 h-16 bg-white/10 border-2 border-white/10 rounded-2xl px-6 text-white font-bold placeholder:text-white/30 outline-none focus:border-blue-400 transition-all text-right"
            />
            <button 
              onClick={handleAiCommand}
              disabled={isAiTyping}
              className="w-20 bg-blue-500 hover:bg-blue-400 text-white rounded-2xl flex items-center justify-center transition-all border-none cursor-pointer"
            >
              {isAiTyping ? <Loader2 className="animate-spin" /> : <Send size={24} />}
            </button>
          </div>
          
          {aiMessages.length > 0 && (
            <div className="mt-6 bg-black/20 rounded-2xl p-4 max-h-40 overflow-y-auto no-scrollbar space-y-2">
              {aiMessages.slice(-3).map((m, i) => (
                <div key={i} className={`p-3 rounded-xl text-sm font-bold ${m.role === 'ai' ? 'bg-blue-500/20 text-blue-200 border-r-4 border-blue-400' : 'text-white/60'}`}>
                  {m.content}
                </div>
              ))}
            </div>
      ...
      {/* 🔥 Mobile AI Drawer (המבורגר) */}
      {showAiMobile && (
        <div className="fixed inset-0 bg-[#0B2C63] z-[200] flex flex-col animate-in slide-in-from-bottom">
          <div className="p-6 flex justify-between items-center border-b border-white/10">
            <div className="flex items-center gap-3">
              <Brain className="text-blue-400" size={24}/>
              <h2 className="text-white font-black italic">SABAN AI</h2>
            </div>
            <button onClick={() => setShowAiMobile(false)} className="bg-white/10 p-2 rounded-full border-none text-white"><X size={24}/></button>
          </div>
          ...
          <div className="p-6 bg-white/5 border-t border-white/10 flex gap-2">
            <input 
              ...
              className="flex-1 h-14 bg-white/10 border-none rounded-xl px-4 text-white font-bold outline-none text-right"
            />
        ...
        <div className="fixed inset-0 bg-[#0B2C63]/95 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <Card className="bg-white w-full max-w-lg rounded-[3rem] p-10 space-y-6 shadow-2xl border-none text-right overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-2xl font-black text-[#0B2C63]">{editingOrder ? 'עריכת הזמנה' : 'הזמנה חדשה'}</h2>
            ...
            <div className="flex gap-2 mb-2">
                {teamMembers.map(m => (
                    <button key={m} onClick={() => setForm({...form, created_by: m})}
                            className={`flex-1 py-4 rounded-2xl text-[11px] font-black border-none transition-all cursor-pointer ${form.created_by === m ? 'bg-blue-600 text-white shadow-xl' : 'bg-slate-100 text-slate-400'}`}>
                        {m}
                    </button>
                ))}
            </div>
            ...
            <input placeholder="שם הלקוח" value={form.customer_name} onChange={e => setForm({...form, customer_name: e.target.value})} className="w-full h-14 px-6 rounded-2xl border-2 border-slate-100 font-bold text-lg text-right outline-none focus:border-blue-500" />
            <input placeholder="מספר קומקס" value={form.order_id_comax} onChange={e => setForm({...form, order_id_comax: e.target.value})} className="w-full h-14 px-6 rounded-2xl border-2 border-slate-100 font-bold text-lg text-right outline-none focus:border-blue-500" />
            
            <div className="space-y-2 text-right">
                <label className="text-[10px] font-black text-slate-400 mr-2 uppercase italic">מחסן מטפל</label>
                <select value={form.warehouse_source} onChange={e => setForm({...form, warehouse_source: e.target.value})} className="w-full h-14 px-6 rounded-2xl border-2 border-slate-100 font-bold text-right outline-none bg-white">
                    {form.driver_name === 'פינוי פסולת' ? wasteWarehouses.map(w => <option key={w} value={w}>{w}</option>) : standardWarehouses.map(w => <option key={w} value={w}>{w}</option>)}
                </select>
            </div>

            <Button onClick={saveOrder} className="w-full h-18 bg-green-600 hover:bg-green-700 text-white rounded-[2rem] font-black text-xl shadow-xl transition-all border-none cursor-pointer">
              שמור סידור 🚀
            </Button>
          </Card>
        </div>
